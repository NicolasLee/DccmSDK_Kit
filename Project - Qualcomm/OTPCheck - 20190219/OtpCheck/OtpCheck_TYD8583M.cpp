...
int OtpCheck_TYD8583M::CheckSum(BOOL flag,int flagaddr, int sumaddr, int datasize, int gap, CString str, int validgroup)//检测多组checksum
{
...
int OtpCheck_TYD8583M::CheckSum_Word(BOOL flag,int flagaddr, int sumaddr, int datasize, int gap, CString str, int validgroup)//检测AFchecksum(MSB LSB)
{
	if (!flag) return FUNCTION_PASS; //没勾选检测对应checksum项则直接返回真
...
void OtpCheck_TYD8583M::SaveOTPData()//点亮后读取模组内的OTP数据
{
