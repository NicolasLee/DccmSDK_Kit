...
BOOL COtpPLQ8579M::CheckSensorOTPEmpty()
{
	...
	if (m_sensoritem == 1)
	{
		...
		if(m_EnBasic)
		{
			...
			{
				m_pInterface->AddLog(_T("OTP内已有Basic烧录数据，无法重复烧录!"),COLOR_RED);		
				m_pInterface->AddLog(_T("开始检测OTP内已有数据!"));	
		...
		if (m_EnAWB)
		{
			...
			{
				m_pInterface->AddLog(_T("OTP内已有AWB烧录数据，无法重复烧录!"),COLOR_RED);		
				m_pInterface->AddLog(_T("开始检测OTP内已有数据!"));	
				m_sensorotpisempty = TRUE;
				return FALSE;
			}
		...
		if (m_EnAF)
		{
			...
			{
				m_pInterface->AddLog(_T("OTP内已有AF烧录数据，无法重复烧录!"),COLOR_RED);		
				m_pInterface->AddLog(_T("开始检测OTP内已有数据!"));	
				m_sensorotpisempty = TRUE;
				return FALSE;
			}
		...
		if(m_EnLSC)
		{
			if ( (tmpdata[0] != 0) ||(tmpdata[1] != 0) || (tmpdata[2] != 0) || (tmpdata[3] != 0) || (tmpdata[4] != 0))
			{
				m_pInterface->AddLog(_T("OTP内已有LSC烧录数据，无法重复烧录!"),COLOR_RED);		
				m_pInterface->AddLog(_T("开始检测OTP内已有数据!"));	
				m_sensorotpisempty = TRUE;
				return FALSE;
			}
...
BOOL COtpPLQ8579M::S5K5E8_CheckOTP()
{
	...
	if (readflag != 1)
	{
		m_pInterface->AddLog(_T("OTP内Flag数值错误!"),COLOR_RED);
		return FALSE;
	}
	...
	{
		m_pInterface->AddLog(_T("Module checksum错误!"),COLOR_RED);
		return FALSE;
	}
	...
	if (calcVal != r_wbsum)
	{
		m_pInterface->AddLog(_T("AWB checksum错误!"),COLOR_RED);
		return FALSE;
	}
	...
	if (calcVal != r_afsum)			
	{
		m_pInterface->AddLog(_T("AF checksum错误!"),COLOR_RED);
		return FALSE;
	}
