#include "stdafx.h"
#include "Otp_TRJ8564.h"
#include "..\..\OTP_SensorSetting.h"

COtpTRJ8564M::COtpTRJ8564M(void)
{
	...
	ZeroMemory(TRJ8564M_LSCWriteData,sizeof(TRJ8564M_LSCWriteData));
...
void COtpTRJ8564M::PreSubclassOtp()
{
...
// 	wbReg->addrGrHigh = TRJ8564M_ADDR_GAIN_GR_H;
// 	wbReg->addrGrLow  = TRJ8564M_ADDR_GAIN_GR_L;
// 	wbReg->addrGbHigh = TRJ8564M_ADDR_GAIN_GB_H;
// 	wbReg->addrGbLow  = TRJ8564M_ADDR_GAIN_GB_L;
// 	wbReg->addrRHigh  = TRJ8564M_ADDR_GAIN_R_H;
// 	wbReg->addrRLow   = TRJ8564M_ADDR_GAIN_R_L;
// 	wbReg->addrBHigh  = TRJ8564M_ADDR_GAIN_B_H;
// 	wbReg->addrBLow   = TRJ8564M_ADDR_GAIN_B_L;
...
// 	lencCtrl->inputTarget = TRJ8564M_VAL_LSC_TARGET;
// 	lencCtrl->inputLow = TRJ8564M_VAL_LSC_LOW;
// 	lencCtrl->inputHigh = TRJ8564M_VAL_LSC_HIGH;
...
// 	lencReg->addrCtrl = TRJ8564M_ADDR_LSC_CTRL;
// 	lencReg->lencEnable = TRJ8564M_ADDR_LSC_EN;
// 	lencReg->lencDisable = TRJ8564M_ADDR_LSC_DIS;
//	lencReg->addrDataStart = REG_INVALID;
//	lencReg->dataSize = TRJ8564M_OTP_LSC_SIZE;
...
// 	expCtrl->target = TRJ8564M_VAL_EXP_TARGET;
// 	expCtrl->lowLimit = TRJ8564M_VAL_EXP_LOW;
// 	expCtrl->highLimit = TRJ8564M_VAL_EXP_HIGH;
...
// 	expReg->addrMid = TRJ8564M_ADDR_EXP_H;
// 	expReg->addrLow = TRJ8564M_ADDR_EXP_L;
...
void COtpTRJ8564M::SetOtpSensorParams()
{
	OtpSensorStruct* params = GetOtpSensorParams();

	params->name = _T("TRJ8564M_MIPI");
...
//	params->otpInit.params = TRJ8564M_OTP_Settings;
//	params->otpInit.paramSize = sizeofTRJ8564M_OTP_Settings;
...
// 	params->verifyInit.params = TRJ8564M_OTP_Settings;
// 	params->verifyInit.paramSize = sizeofTRJ8564M_OTP_Settings;	
}


BOOL COtpTRJ8564M::EraseAllData()
{
	DW9763_All_Erase();
...
void COtpTRJ8564M::SetWBDefaultGain(void)
{	
	return S5K3L8_SetDefaultAwb();
}

BOOL COtpTRJ8564M::StartProgram()
{
	return TRUE;
}

BOOL COtpTRJ8564M::WriteOTP()
{
	TRJ8564M_GetBasicData();
	TRJ8564M_GetAFData();
	CreateBurningData();
	if (!m_dummyburn)
	{
		if (!TRJ8564M_OtpWrite())//
		{return FALSE;}
	}
	else
	{
...
BOOL COtpTRJ8564M::WriteOTP(vector<double>& vData)
{
	TRJ8564M_GetBasicData();
	TRJ8564M_GetAFData();
	CreateBurningData();
...
void COtpTRJ8564M::SaveOTPData(vector<double>& vData)
{
	m_pInterface->AddLog(_T("Read OTP Start!"));
	///////////////////////////////Read OTP////////////////////////////////////
	int valSize = 481;
	...
	if (m_LscGroup == 1)
	{
		...
		for (i=0; i<417; i++)
		{
			pVal[i+64] = TRJ8564M_LSCWriteData[i+31];
		}
	...
	else if (m_LscGroup == 2)
	{
		...
		for (i=0; i<417; i++)
		{
			pVal[i+64] = TRJ8564M_LSCWriteData[i+62];
		}       
	}
...
void COtpTRJ8564M::CreateBurningData(void)
{
	int i = 0;
	//////////////////////////////////Basic//////////////////////////////////////
	...
	if (m_LSCItem != 2)
	{
		if (m_LSCItem == 0)
		{
			for (i = 0; i < SIZE_LSC_GROUP-1; i++)
			{
				Data_Write.data_Lenc[i+1] = TRJ8564M_LSCWriteData[i] = m_mtklscdata[i];
			}
		}
		else	if (m_LSCItem == 1)
		{
			for (i = 0; i < SIZE_LSC_GROUP-1; i++)
			{
				Data_Write.data_Lenc[i+1] = TRJ8564M_LSCWriteData[i] = m_quallscdata[i];
			}
		}
	...
	//////////////////////////////////////////////////////////////////////////
	TRJ8564M_CalcAllCheckSum();
	return;
}

BOOL COtpTRJ8564M::CheckOTP()
{
	if (!TRJ8564M_OTPVerify())		return FALSE;

	return TRUE;
}

BOOL COtpTRJ8564M::VerifyLSC()
{
	if (m_LSCItem == 2)
	{
		if (!TRJ8564M_VerifyLSC())		return FALSE; //for sensor LSC
	}
	return TRUE;
}

BOOL COtpTRJ8564M::VerifyPreWB()
{
	if (m_EnAWB)
	{
...
BOOL COtpTRJ8564M::VerifyAWB()
{
	if (m_EnAWB)
	{
		if (!TRJ8564M_VerifyAWB())		return FALSE;
	}
	return TRUE;
}

// BOOL COtpTRJ8564M::EndProgram()
// {
// 	return TRUE;
// }


BOOL COtpTRJ8564M::GetAWB()
{
	if (m_EnAWB)
	{
		if (!TRJ8564M_GetAWBData(OTP_Data_Write))
		{
			return FALSE;
		}
	}
...
BOOL COtpTRJ8564M::TRJ8564M_GetAWBData(TRJ8564MOTPDataSpec &awbdata)
{
	m_pInterface ->AddLog(_T("Get AWB Data Start"));

	if (!TRJ8564M_GetCurrentAWB(&m_wbCurrent))		return FALSE;
...
BOOL COtpTRJ8564M::CheckOTPAWB()
{
	if (m_EnAWB)
	{
		...
		if (!TRJ8564M_CheckAWB(m_wbCurrent,1))				return FALSE;
...
BOOL COtpTRJ8564M::TRJ8564M_GetCurrentAWB(tagAwbRatio* wb)
{
	if (!InitAWB())		return FALSE;

	if (!GetAWBInfo(&m_wbCurrent))		return FALSE;

	if (!TRJ8564M_CheckAWB(m_wbCurrent,0))				return FALSE;
...
BOOL COtpTRJ8564M::GetAWBInfo(tagAwbRatio* wb)
{
	// getwb
...
BOOL COtpTRJ8564M::TRJ8564M_CheckAWB(tagAwbRatio wb, int type)
{
	BOOL bRet = TRUE;
...
void COtpTRJ8564M::TRJ8564M_GetBasicData(void)
{
...
void COtpTRJ8564M::TRJ8564M_GetAFData(void)
{
...
void COtpTRJ8564M::TRJ8564M_CalcAllCheckSum(void)
{
	int i = 0;
	int sum = 0;
	...
	if (m_EnLSC)
	{
		if (m_LSCItem != 2)
		{
			sum = SumCalc(TRJ8564M_LSCWriteData, SIZE_LSC_GROUP-1, 255,1);
...
BOOL COtpTRJ8564M::TRJ8564M_OtpWrite(void)
{
...
BOOL COtpTRJ8564M::ProgrammingBasic()
{
	if (m_LSCItem == 0)
	{
...
void COtpTRJ8564M::Programming_3L8Basic()
{
	int i = 0;
	BYTE WriteData[64] = {0};
	if (m_LscGroup == 1)
...
BOOL COtpTRJ8564M::ProgrammingAWB()
{
	if (m_LSCItem != 2)
	{
...
void COtpTRJ8564M::Programming_3L8AWB()
{
	int i = 0;
	BYTE WriteData[64] = {0};
	if (m_LscGroup == 1)
...
BOOL COtpTRJ8564M::ProgrammingLSC()
{
	if (m_LSCItem != 2)
	{
...
BOOL COtpTRJ8564M::Programming_3L8LSC()
{
	BYTE _LSCData[417] = {0};
		...
		if ( strLine.Find( _T("s"), 0 ) == 0 )
		{
			...
			else//写入数据
			{
				...
				for(i = 0; i < nLength2; i++ )
				{
					...
					TRJ8564M_LSCWriteData[k] = ParaList[i];
					k++;
				}
			}
		}
	...
	//calc lsc checksum
	TRJ8564M_LSCchecksum=0;
	if(m_LscGroup==1)
	{
		for (i=0;i<417;i++)
		{
			TRJ8564M_LSCchecksum+=TRJ8564M_LSCWriteData[i+31];
		}
		TRJ8564M_LSCchecksum=TRJ8564M_LSCchecksum%255+1;
	}
	if (m_LscGroup==2)
	{
		for (i=0;i<417;i++)
		{
			TRJ8564M_LSCchecksum+=TRJ8564M_LSCWriteData[i+62];
		}
		TRJ8564M_LSCchecksum=TRJ8564M_LSCchecksum%255+1;
	}

	TRJ8564M_WriteLSCFlagSum();

	return TRUE;
}

void COtpTRJ8564M::TRJ8564M_WriteLSCFlagSum()
{
	Data_Write.data_Lenc[0] = FLAG_VALID;
	Data_Write.data_Lenc[1] = TRJ8564M_LSCchecksum;
...
BOOL COtpTRJ8564M::ProgrammingAF()
{
	if (m_LSCItem != 2)
	{
...
void COtpTRJ8564M::Programming_3L8AF()
{
	int i = 0;
	BYTE WriteData[64] = {0};
	if (m_LscGroup == 1)
...
BOOL COtpTRJ8564M::ProgrammingPDAF()
{
	BOOL bRet = TRUE;
	//MTK
	if (m_EnPDAF)				
...
BOOL COtpTRJ8564M::TRJ8564M_OTPVerify(void)
{
	if (!m_dummyburn)//!m_dummyburn
	{
		if (!TRJ8564M_CheckOTPReadWrite())		return FALSE;
	}
	else 
	{
		if (m_LSCItem == 2)
...
BOOL COtpTRJ8564M::TRJ8564M_CheckOTPReadWrite(void)
{
...
BOOL COtpTRJ8564M::Sensor3L8_CheckReadWrite()
{
	BYTE ReadData[64] = {0};
...
BOOL COtpTRJ8564M::Sensor3L8_CheckLSCReadWrite()
{
	int i = 0;
	if (m_LscGroup == 1)
	{
		S5K3L8_ReadOTPPage(0x19,TRJ8564M_LSCReadData);
		msleep(100);
		S5K3L8_ReadOTPPage(0x1A,TRJ8564M_LSCReadData+64);
		msleep(100);
		S5K3L8_ReadOTPPage(0x1B,TRJ8564M_LSCReadData+128);
		msleep(100);
		S5K3L8_ReadOTPPage(0x1C,TRJ8564M_LSCReadData+192);
		msleep(100);
		S5K3L8_ReadOTPPage(0x1D,TRJ8564M_LSCReadData+256);
		msleep(100);
		S5K3L8_ReadOTPPage(0x1E,TRJ8564M_LSCReadData+320);
		msleep(100);
		S5K3L8_ReadOTPPage(0x1F,TRJ8564M_LSCReadData+384);
		msleep(100);	
		for (i=31;i<(417+31);i++)
		{	
			if (TRJ8564M_LSCReadData[i] != TRJ8564M_LSCWriteData[i])
			{
	...
	if (m_LscGroup == 2)
	{
		S5K3L8_ReadOTPPage(0x12,TRJ8564M_LSCReadData);
		msleep(100);
		S5K3L8_ReadOTPPage(0x13,TRJ8564M_LSCReadData+64);
		msleep(100);
		S5K3L8_ReadOTPPage(0x14,TRJ8564M_LSCReadData+128);
		msleep(100);
		S5K3L8_ReadOTPPage(0x15,TRJ8564M_LSCReadData+192);
		msleep(100);
		S5K3L8_ReadOTPPage(0x16,TRJ8564M_LSCReadData+256);
		msleep(100);
		S5K3L8_ReadOTPPage(0x17,TRJ8564M_LSCReadData+320);
		msleep(100);
		S5K3L8_ReadOTPPage(0x18,TRJ8564M_LSCReadData+384);
		msleep(100);
		S5K3L8_ReadOTPPage(0x19,TRJ8564M_LSCReadData+448);
		msleep(100);
		for (i=62;i<(417+62);i++)
		{	
			if (TRJ8564M_LSCReadData[i] != TRJ8564M_LSCWriteData[i])
...
BOOL COtpTRJ8564M::TRJ8564M_VerifyLSC()
{
...
BOOL COtpTRJ8564M::TRJ8564M_VerifyAWB()
{
	if (!GetAWBInfo(&m_wbCurrent))		return FALSE;
	if (!TRJ8564M_CheckAWB(m_wbCurrent,2))		return FALSE;
	return TRUE;
}