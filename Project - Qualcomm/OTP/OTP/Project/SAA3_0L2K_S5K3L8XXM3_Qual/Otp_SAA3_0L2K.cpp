#include "stdafx.h"
#include "Otp_SAA3_0L2K.h"
#include "..\..\OTP_SensorSetting.h"

COtpSAA3_0L2K::COtpSAA3_0L2K(void)
{
	...
	ZeroMemory(SAA3_0L2K_LSCWriteData,sizeof(SAA3_0L2K_LSCWriteData));
	...
	m_addrFlag.pdaf			    = REG_PDAF_PROC1_FLAG;
	m_addrFlag.pdaf2				= REG_PDAF_PROC2_FLAG; // Proc2 First Data的上一个地址
	...
	m_addrSum.pdaf = REG_PDAF_PROC1_CHECKSUM;
...
void COtpSAA3_0L2K::PreSubclassOtp()
{
...
// 	wbReg->addrGrHigh = SAA3_0L2K_ADDR_GAIN_GR_H;
// 	wbReg->addrGrLow  = SAA3_0L2K_ADDR_GAIN_GR_L;
// 	wbReg->addrGbHigh = SAA3_0L2K_ADDR_GAIN_GB_H;
// 	wbReg->addrGbLow  = SAA3_0L2K_ADDR_GAIN_GB_L;
// 	wbReg->addrRHigh  = SAA3_0L2K_ADDR_GAIN_R_H;
// 	wbReg->addrRLow   = SAA3_0L2K_ADDR_GAIN_R_L;
// 	wbReg->addrBHigh  = SAA3_0L2K_ADDR_GAIN_B_H;
// 	wbReg->addrBLow   = SAA3_0L2K_ADDR_GAIN_B_L;
...
// 	lencCtrl->inputTarget = SAA3_0L2K_VAL_LSC_TARGET;
// 	lencCtrl->inputLow = SAA3_0L2K_VAL_LSC_LOW;
// 	lencCtrl->inputHigh = SAA3_0L2K_VAL_LSC_HIGH;
...
// 	lencReg->addrCtrl = SAA3_0L2K_ADDR_LSC_CTRL;
// 	lencReg->lencEnable = SAA3_0L2K_ADDR_LSC_EN;
// 	lencReg->lencDisable = SAA3_0L2K_ADDR_LSC_DIS;
//	lencReg->addrDataStart = REG_INVALID;
//	lencReg->dataSize = SAA3_0L2K_OTP_LSC_SIZE;
...
// 	expCtrl->target = SAA3_0L2K_VAL_EXP_TARGET;
// 	expCtrl->lowLimit = SAA3_0L2K_VAL_EXP_LOW;
// 	expCtrl->highLimit = SAA3_0L2K_VAL_EXP_HIGH;
...
// 	expReg->addrMid = SAA3_0L2K_ADDR_EXP_H;
// 	expReg->addrLow = SAA3_0L2K_ADDR_EXP_L;
	...
	mtkpdaf->Pdaf_FlagAddr = REG_PDAF_PROC1_FLAG;
	mtkpdaf->Proc1_FirstAddr = REG_PDAF_PROC1_FLAG+1;
	mtkpdaf->Proc1Size= SIZE_PDAF_GROUP1;
	mtkpdaf->Proc2_FirstAddr =  REG_PDAF_PROC2_FLAG+1;
	mtkpdaf->Proc2Size = SIZE_PDAF_GROUP2;
	mtkpdaf->TotalSize = SIZE_PDAF_GROUP1 + SIZE_PDAF_GROUP2;
	mtkpdaf->Proc1_SumAddr =  REG_PDAF_PROC1_CHECKSUM;
	mtkpdaf->Proc2_SumAddr = REG_PDAF_PROC2_CHECKSUM;
...
void COtpSAA3_0L2K::SetOtpSensorParams()
{
	OtpSensorStruct* params = GetOtpSensorParams();

	params->name = _T("SAA3_0L2K_MIPI");
...
//	params->otpInit.params = SAA3_0L2K_OTP_Settings;
//	params->otpInit.paramSize = sizeofSAA3_0L2K_OTP_Settings;
...
// 	params->verifyInit.params = SAA3_0L2K_OTP_Settings;
// 	params->verifyInit.paramSize = sizeofSAA3_0L2K_OTP_Settings;	
}


BOOL COtpSAA3_0L2K::EraseAllData()
{
	DW9763_All_Erase();
...
void COtpSAA3_0L2K::SetWBDefaultGain(void)
{	
	return S5K3L8_SetDefaultAwb();
}

BOOL COtpSAA3_0L2K::StartProgram()
{
	return TRUE;
}

BOOL COtpSAA3_0L2K::WriteOTP()
{
	SAA3_0L2K_GetBasicData();
	SAA3_0L2K_GetAFData();
	CreateBurningData();
	if (!m_dummyburn)
	{
		if (!SAA3_0L2K_OtpWrite())//
		{return FALSE;}
	}
	else
	{
...
void COtpSAA3_0L2K::SaveOTPData()
{
...
BOOL COtpSAA3_0L2K::WriteOTP(vector<double>& vData)
{
	SAA3_0L2K_GetBasicData();
	SAA3_0L2K_GetAFData();
	CreateBurningData();
	if (!m_dummyburn)
	{
		if (!SAA3_0L2K_OtpWrite()&& m_sensorotpisempty ==FALSE)//
		{
			return FALSE;
		}
	}
...
void COtpSAA3_0L2K::SaveOTPData(vector<double>& vData)
{
	m_pInterface->AddLog(_T("Read OTP Start!"));
	///////////////////////////////Read OTP////////////////////////////////////
	int valSize = 481;
...
// 		}
		...
		for (i=0; i<417; i++)
		{
			pVal[i+64] = SAA3_0L2K_LSCWriteData[i+31];
		}
...
// 		}
		...
		for (i=0; i<417; i++)
		{
			pVal[i+64] = SAA3_0L2K_LSCWriteData[i+62];
		}       
	}
...
void COtpSAA3_0L2K::CreateBurningData(void)
{
	int i = 0;
	//////////////////////////////////Basic//////////////////////////////////////
...
BOOL COtpSAA3_0L2K::CheckOTP()
{
	if (!SAA3_0L2K_OTPVerify())		return FALSE;

	return TRUE;
}

BOOL COtpSAA3_0L2K::VerifyLSC()
{
	if (m_LSCItem == 2)
	{
		if (!SAA3_0L2K_VerifyLSC())		return FALSE; //for sensor LSC
	}
	return TRUE;
}

BOOL COtpSAA3_0L2K::VerifyPreWB()
{
	if (m_EnAWB)
	{
...
BOOL COtpSAA3_0L2K::VerifyAWB()
{
	if (m_EnAWB)
	{
		if (!SAA3_0L2K_VerifyAWB())		return FALSE;
	}
	return TRUE;
}

// BOOL COtpSAA3_0L2K::EndProgram()
// {
// 	return TRUE;
// }


BOOL COtpSAA3_0L2K::GetAWB()
{
	if (m_EnAWB)
	{
		if (!SAA3_0L2K_GetAWBData(OTP_Data_Write))
		{
			return FALSE;
		}
	}
...
BOOL COtpSAA3_0L2K::SAA3_0L2K_GetAWBData(SAA3_0L2K_OTPDataSpec &awbdata)
{
	m_pInterface ->AddLog(_T("Get AWB Data Start"));

	if (!SAA3_0L2K_GetCurrentAWB(&m_wbCurrent))		return FALSE;
...
BOOL COtpSAA3_0L2K::CheckOTPAWB()
{
	if (m_EnAWB)
	{
		...
		if (!SAA3_0L2K_CheckAWB(m_wbCurrent,1))				return FALSE;
...
BOOL COtpSAA3_0L2K::SAA3_0L2K_GetCurrentAWB(tagAwbRatio* wb)
{
	if (!InitAWB())		return FALSE;

	if (!GetAWBInfo(&m_wbCurrent))		return FALSE;

	if (!SAA3_0L2K_CheckAWB(m_wbCurrent,0))				return FALSE;
...
BOOL COtpSAA3_0L2K::GetAWBInfo(tagAwbRatio* wb)
{
	// getwb
...
BOOL COtpSAA3_0L2K::SAA3_0L2K_CheckAWB(tagAwbRatio wb, int type)
{
	BOOL bRet = TRUE;
...
void COtpSAA3_0L2K::SAA3_0L2K_GetBasicData(void)
{
	//	m_pInterface ->AddLog(_T("Get Basic Data Start"));
	CTime time = CTime::GetCurrentTime();

	int i = 0;
	OTP_Data_Write.BaseInfo[i++] = BASIC_MID;
	OTP_Data_Write.BaseInfo[i++] = time.GetYear() - 2000;
	OTP_Data_Write.BaseInfo[i++] = time.GetMonth();
	OTP_Data_Write.BaseInfo[i++] = time.GetDay();
	OTP_Data_Write.BaseInfo[i++] = BASIC_LENSID;
	OTP_Data_Write.BaseInfo[i++] = BASIC_DRIVERICID;
	OTP_Data_Write.BaseInfo[i++] = BASIC_VCMID;
...
void COtpSAA3_0L2K::SAA3_0L2K_GetAFData(void)
{
//	m_pInterface ->AddLog(_T("Get AF Data Start"));
	int i = 0;
	OTP_Data_Write.AFInfo[i++] = 0x02;
	OTP_Data_Write.AFInfo[i++] = (m_infCode>>8)&0x0f;
	OTP_Data_Write.AFInfo[i++] = m_infCode & 0xff;
	OTP_Data_Write.AFInfo[i++] = (m_macCode >> 8) & 0x0f;
	OTP_Data_Write.AFInfo[i++] = m_macCode & 0xff;
	OTP_Data_Write.AFInfo[i++] = (m_stcCode >> 8) & 0x0f;
...
{
	// checksum formula sum(from m to n) % mod + add
	int i = 0;
	int sum = 0;
	int add = 0;
	...
	if (m_LSCItem != 2)
	{
		sum = SumCalc(OTP_Data_Write.LSCInfo, SIZE_LSC_GROUP - 1, 255, add);
	...
	//AF	
	sum = SumCalc(OTP_Data_Write.AFInfo, SIZE_AF_GROUP - 1, 255, add);
	Data_Write.data_AF[SIZE_AF_GROUP] = sum;

	if (m_EnPDAF)
	{
		//PDAF1
		sum = SumCalc(OTP_Data_Write.PDAF1, SIZE_PDAF_GROUP1 - 1, 255, add);
		Data_Write.data_PDAF[SIZE_PDAF_GROUP1] = sum;//checksum在reserve前面
	}
	return;
}
BOOL COtpSAA3_0L2K::CheckSensorOTPEmpty()
{
	BYTE tmpdata[64] = {0};
	if (m_sensoritem == 0)
	{		
...
BOOL COtpSAA3_0L2K::SAA3_0L2K_OtpWrite(void)
{
...
BOOL COtpSAA3_0L2K::ProgrammingBasic()
{
	if (m_LSCItem == 1)
	{
...
BOOL COtpSAA3_0L2K::Programming_3L8Basic()
{
	int i = 0;
	BYTE WriteData[64] = {0};
	BYTE ReadData[64] = {0};
...
BOOL COtpSAA3_0L2K::ProgrammingAWB()
{
	if (m_LSCItem != 2)
	{
...
void COtpSAA3_0L2K::Programming_3L8AWB()
{
	int i = 0;
	BYTE WriteData[64] = {0};
	if (m_LscGroup == 1)
...
BOOL COtpSAA3_0L2K::ProgrammingLSC()
{
	if (m_LSCItem != 2)
	{
...
BOOL COtpSAA3_0L2K::Programming_3L8LSC()
{
	if (!m_dummyburn)
	{
		BYTE _LSCData[64] = {0};
		...
		if ( strLine.Find( _T("s"), 0 ) == 0 )
		{
			...
			else//写入数据
			{
				...
				for(i = 0; i < nLength2; i++ )
				{
					...
					SAA3_0L2K_LSCWriteData[k] = ParaList[i];
					k++;
				}
			}
		}
	...
	//calc lsc checksum
	SAA3_0L2K_LSCchecksum=0;
	if(m_LscGroup==1)
	{
		for (i=0;i<417;i++)
		{
			SAA3_0L2K_LSCchecksum+=SAA3_0L2K_LSCWriteData[i+31];
		}
		SAA3_0L2K_LSCchecksum=SAA3_0L2K_LSCchecksum%255+1;
	}
	if (m_LscGroup==2)
	{
		for (i=0;i<417;i++)
		{
			SAA3_0L2K_LSCchecksum+=SAA3_0L2K_LSCWriteData[i+62];
		}
		SAA3_0L2K_LSCchecksum=SAA3_0L2K_LSCchecksum%255+1;
	}

//	SAA3_0L2K_WriteLSCFlagSum();

	return TRUE;
}

void COtpSAA3_0L2K::SAA3_0L2K_WriteLSCFlagSum()
{
	Data_Write.data_Lenc[0] = FLAG_VALID;
	Data_Write.data_Lenc[1] = SAA3_0L2K_LSCchecksum;
...
BOOL COtpSAA3_0L2K::ProgrammingAF()
{
	if (m_LSCItem != 2)
	{
...
void COtpSAA3_0L2K::Programming_3L8AF()
{
	int i = 0;
	BYTE WriteData[64] = {0};
	if (m_LscGroup == 1)
...
BOOL COtpSAA3_0L2K::ProgrammingPDAF()
{
	BOOL bRet = TRUE;
	if (m_EnPDAF)
	{

		if (GT24C64_Programming(m_addrFlag.pdaf, Data_Write.data_PDAF1, sizeof(Data_Write.data_PDAF1)) != TRUE)
		{
			m_pInterface->AddLog(_T("PDAF Info Write Fail!"), COLOR_RED);
			return FALSE;
		}
		msleep(10);
	}
...
BOOL COtpSAA3_0L2K::SAA3_0L2K_CheckOTP()
{
	Sensor3L8_StreamOn();
	msleep(100);
	BYTE otpdata[64] = {0};
	...
	{
		m_pInterface->AddLog(_T("OTP内Basic Flag为空!"),COLOR_RED);
		return FALSE;
	}
	...
	{
		m_pInterface->AddLog(_T("OTP内AF Flag为空!"),COLOR_RED);
		return FALSE;
	}
	...
	{
		m_pInterface->AddLog(_T("AF Flag异常!"),COLOR_RED);
		return FALSE;
	}
...
BOOL COtpSAA3_0L2K::SAA3_0L2K_OTPVerify(void)
{
	if (!m_dummyburn)//!m_dummyburn
	{
		if (!m_sensorotpisempty)
		{
			if (!SAA3_0L2K_CheckOTPReadWrite())		return FALSE;
		}
		else
		{
			if (!SAA3_0L2K_CheckOTP())		return FALSE;
		}
...
BOOL COtpSAA3_0L2K::SAA3_0L2K_CheckOTPReadWrite(void)
{
	...
	{	
		...
		if (m_EnPDAF)
		{
			if (GT24C64_Read(m_addrFlag.pdaf, Data_Read.data_PDAF1, sizeof(Data_Read.data_PDAF1)) != TRUE)
			{
...
BOOL COtpSAA3_0L2K::Sensor3L8_CheckReadWrite()
{
	BYTE ReadData[64] = {0};
	S5K3L8_ReadOTPPage(1,ReadData);
...
BOOL COtpSAA3_0L2K::Sensor3L8_CheckLSCReadWrite()
{
	int i = 0;
	if (m_LscGroup == 1)
	{
		S5K3L8_ReadOTPPage(0x19,SAA3_0L2K_LSCReadData);
		msleep(100);
		S5K3L8_ReadOTPPage(0x1A,SAA3_0L2K_LSCReadData+64);
		msleep(100);
		S5K3L8_ReadOTPPage(0x1B,SAA3_0L2K_LSCReadData+128);
		msleep(100);
		S5K3L8_ReadOTPPage(0x1C,SAA3_0L2K_LSCReadData+192);
		msleep(100);
		S5K3L8_ReadOTPPage(0x1D,SAA3_0L2K_LSCReadData+256);
		msleep(100);
		S5K3L8_ReadOTPPage(0x1E,SAA3_0L2K_LSCReadData+320);
		msleep(100);
		S5K3L8_ReadOTPPage(0x1F,SAA3_0L2K_LSCReadData+384);
		msleep(100);	
		for (i=31;i<(417+31);i++)
		{	
			if (SAA3_0L2K_LSCReadData[i] != SAA3_0L2K_LSCWriteData[i])
			{
	...
	if (m_LscGroup == 2)
	{
		S5K3L8_ReadOTPPage(0x12,SAA3_0L2K_LSCReadData);
		msleep(100);
		S5K3L8_ReadOTPPage(0x13,SAA3_0L2K_LSCReadData+64);
		msleep(100);
		S5K3L8_ReadOTPPage(0x14,SAA3_0L2K_LSCReadData+128);
		msleep(100);
		S5K3L8_ReadOTPPage(0x15,SAA3_0L2K_LSCReadData+192);
		msleep(100);
		S5K3L8_ReadOTPPage(0x16,SAA3_0L2K_LSCReadData+256);
		msleep(100);
		S5K3L8_ReadOTPPage(0x17,SAA3_0L2K_LSCReadData+320);
		msleep(100);
		S5K3L8_ReadOTPPage(0x18,SAA3_0L2K_LSCReadData+384);
		msleep(100);
		S5K3L8_ReadOTPPage(0x19,SAA3_0L2K_LSCReadData+448);
		msleep(100);
		for (i=62;i<(417+62);i++)
		{	
			if (SAA3_0L2K_LSCReadData[i] != SAA3_0L2K_LSCWriteData[i])
...
BOOL COtpSAA3_0L2K::SAA3_0L2K_VerifyLSC()
{
...
BOOL COtpSAA3_0L2K::SAA3_0L2K_VerifyAWB()
{
	if (!GetAWBInfo(&m_wbCurrent))		return FALSE;
	if (!SAA3_0L2K_CheckAWB(m_wbCurrent,2))		return FALSE;
	return TRUE;
}