...
class COtpdefineCB801C : public COtpBase
{
public:
	enum normal
	{
	...
	enum S5K4H7_OTP_SIZE
	{
		...
		SIZE_TOTAL_GROUP = 64, //第一个Data到最后一个Data
	};

	enum S5K4H7_REG_ADDR
	{
		GAIN_ADDR_GR = 0x020e,
		GAIN_ADDR_R  = 0x0210,
		GAIN_ADDR_B  = 0x0212,
	...
	typedef struct _S5K4H7OTPDataSpec//包含数据和Checksum，不包含Flag
	{
	...
	typedef union S5K4H7EEPROMOtpGroupUnion
	{
...
// 				BYTE  wb_goldenGr_H;
			...
			union
			{
				BYTE data_Lenc[SIZE_LSC_GROUP];//SIZE_LSC_GROUP+1
				struct
				{
					// 				BYTE lsc_flag;
