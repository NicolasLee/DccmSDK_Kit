#include "stdafx.h"
#include "Otp_TYD8635M.h"
#include "..\..\OTP_SensorSetting.h"

COtpTYD8635M::COtpTYD8635M(void)
{
	...
	ZeroMemory(TYD8635M_LSCWriteData,sizeof(TYD8635M_LSCWriteData));
...
void COtpTYD8635M::PreSubclassOtp()
{
...
// 	wbReg->addrGrHigh = TYD8635M_ADDR_GAIN_GR_H;
// 	wbReg->addrGrLow  = TYD8635M_ADDR_GAIN_GR_L;
// 	wbReg->addrGbHigh = TYD8635M_ADDR_GAIN_GB_H;
// 	wbReg->addrGbLow  = TYD8635M_ADDR_GAIN_GB_L;
// 	wbReg->addrRHigh  = TYD8635M_ADDR_GAIN_R_H;
// 	wbReg->addrRLow   = TYD8635M_ADDR_GAIN_R_L;
// 	wbReg->addrBHigh  = TYD8635M_ADDR_GAIN_B_H;
// 	wbReg->addrBLow   = TYD8635M_ADDR_GAIN_B_L;
...
// 	lencCtrl->inputTarget = TYD8635M_VAL_LSC_TARGET;
// 	lencCtrl->inputLow = TYD8635M_VAL_LSC_LOW;
// 	lencCtrl->inputHigh = TYD8635M_VAL_LSC_HIGH;
...
// 	lencReg->addrCtrl = TYD8635M_ADDR_LSC_CTRL;
// 	lencReg->lencEnable = TYD8635M_ADDR_LSC_EN;
// 	lencReg->lencDisable = TYD8635M_ADDR_LSC_DIS;
//	lencReg->addrDataStart = REG_INVALID;
//	lencReg->dataSize = TYD8635M_OTP_LSC_SIZE;
...
// 	expCtrl->target = TYD8635M_VAL_EXP_TARGET;
// 	expCtrl->lowLimit = TYD8635M_VAL_EXP_LOW;
// 	expCtrl->highLimit = TYD8635M_VAL_EXP_HIGH;
...
// 	expReg->addrMid = TYD8635M_ADDR_EXP_H;
// 	expReg->addrLow = TYD8635M_ADDR_EXP_L;
...
void COtpTYD8635M::SetOtpSensorParams()
{
	OtpSensorStruct* params = GetOtpSensorParams();

	params->name = _T("TYD8635M_MIPI");
...
//	params->otpInit.params = TYD8635M_OTP_Settings;
//	params->otpInit.paramSize = sizeofTYD8635M_OTP_Settings;
...
// 	params->verifyInit.params = TYD8635M_OTP_Settings;
// 	params->verifyInit.paramSize = sizeofTYD8635M_OTP_Settings;	
}


BOOL COtpTYD8635M::EraseAllData()
{
	DW9763_All_Erase();
...
void COtpTYD8635M::SetWBDefaultGain(void)
{	
	return OV13855_SetDefaultAwb();
}

BOOL COtpTYD8635M::StartProgram()
{
	return TRUE;
}

BOOL COtpTYD8635M::WriteOTP()
{
	TYD8635M_GetBasicData();
	TYD8635M_GetAFData();
	CreateBurningData();
	if (!m_dummyburn)
	{
		if (!TYD8635M_OtpWrite())//
		{return FALSE;}
	}
	else
	{
...
BOOL COtpTYD8635M::WriteOTP(vector<double>& vData)
{
	TYD8635M_GetBasicData();
	TYD8635M_GetAFData();
	CreateBurningData();
...
void COtpTYD8635M::SaveOTPData(vector<double>& vData)
{
	m_pInterface->AddLog(_T("Read OTP Start!"));
	///////////////////////////////Read OTP////////////////////////////////////
...
void COtpTYD8635M::CreateBurningData(void)
{
	int i = 0;
	//////////////////////////////////Basic//////////////////////////////////////
	...
	if (m_LSCItem != 2)
	{
		if (m_LSCItem == 0)
		{
			for (i = 0; i < SIZE_LSC_GROUP-1; i++)
			{
				Data_Write.data_Lenc[i+1] = TYD8635M_LSCWriteData[i] = m_mtklscdata[i];
			}
		}
		else	if (m_LSCItem == 1)
		{
			for (i = 0; i < SIZE_LSC_GROUP-1; i++)
			{
				Data_Write.data_Lenc[i+1] = TYD8635M_LSCWriteData[i] = m_quallscdata[i];
			}
		}
	...
	//////////////////////////////////////////////////////////////////////////
	TYD8635M_CalcAllCheckSum();
	return;
}

BOOL COtpTYD8635M::CheckOTP()
{
	if (!TYD8635M_OTPVerify())		return FALSE;

	return TRUE;
}

BOOL COtpTYD8635M::VerifyLSC()
{
	if (m_LSCItem == 2)
	{
		if (!TYD8635M_VerifyLSC())		return FALSE; //for sensor LSC
	}
	return TRUE;
}

BOOL COtpTYD8635M::VerifyPreWB()
{
	if (m_EnAWB)
	{
...
BOOL COtpTYD8635M::VerifyAWB()
{
	if (m_EnAWB)
	{
		if (!TYD8635M_VerifyAWB())		return FALSE;
	}
	return TRUE;
}

// BOOL COtpTYD8635M::EndProgram()
// {
// 	return TRUE;
// }


BOOL COtpTYD8635M::GetAWB()
{
	if (m_EnAWB)
	{
		if (!TYD8635M_GetAWBData(OTP_Data_Write))
		{
			return FALSE;
		}
	}
...
BOOL COtpTYD8635M::TYD8635M_GetAWBData(TYD8635MOTPDataSpec &awbdata)
{
	m_pInterface ->AddLog(_T("Get AWB Data Start"));

	if (!TYD8635M_GetCurrentAWB(&m_wbCurrent))		return FALSE;
...
BOOL COtpTYD8635M::CheckOTPAWB()
{
	if (m_EnAWB)
	{
		...
		if (!TYD8635M_CheckAWB(m_wbCurrent,1))				return FALSE;
...
BOOL COtpTYD8635M::TYD8635M_GetCurrentAWB(tagAwbRatio* wb)
{
	if (!InitAWB())		return FALSE;

	if (!GetAWBInfo(&m_wbCurrent))		return FALSE;

	if (!TYD8635M_CheckAWB(m_wbCurrent,0))				return FALSE;
...
BOOL COtpTYD8635M::GetAWBInfo(tagAwbRatio* wb)
{
	// getwb
...
BOOL COtpTYD8635M::TYD8635M_CheckAWB(tagAwbRatio wb, int type)
{
	BOOL bRet = TRUE;
...
void COtpTYD8635M::TYD8635M_GetBasicData(void)
{
...
void COtpTYD8635M::TYD8635M_GetAFData(void)
{
...
void COtpTYD8635M::TYD8635M_CalcAllCheckSum(void)
{
	int i = 0;
	int sum = 0;
	...
	if (m_EnLSC)
	{
		if (m_LSCItem != 2)
		{
			sum = SumCalc(TYD8635M_LSCWriteData, SIZE_LSC_GROUP-1, 255,1);
...
BOOL COtpTYD8635M::TYD8635M_OtpWrite(void)
{
...
BOOL COtpTYD8635M::ProgrammingBasic()
{
	if (m_LSCItem == 0)
	{
...
void COtpTYD8635M::Programming_3L8Basic()
{
	int i = 0;
	BYTE WriteData[64] = {0};
	if (m_LscGroup == 1)
...
BOOL COtpTYD8635M::ProgrammingAWB()
{
	if (m_LSCItem != 2)
	{
...
void COtpTYD8635M::Programming_3L8AWB()
{
	int i = 0;
	BYTE WriteData[64] = {0};
	if (m_LscGroup == 1)
...
BOOL COtpTYD8635M::ProgrammingLSC()
{
	if (m_LSCItem != 2)
	{
...
BOOL COtpTYD8635M::Programming_3L8LSC()
{
	BYTE _LSCData[417] = {0};
		...
		if ( strLine.Find( _T("s"), 0 ) == 0 )
		{
			...
			else//写入数据
			{
				...
				for(i = 0; i < nLength2; i++ )
				{
					...
					TYD8635M_LSCWriteData[k] = ParaList[i];
					k++;
				}
			}
		}
	...
	//calc lsc checksum
	TYD8635M_LSCchecksum=0;
	if(m_LscGroup==1)
	{
		for (i=0;i<417;i++)
		{
			TYD8635M_LSCchecksum+=TYD8635M_LSCWriteData[i+31];
		}
		TYD8635M_LSCchecksum=TYD8635M_LSCchecksum%255+1;
	}
	if (m_LscGroup==2)
	{
		for (i=0;i<417;i++)
		{
			TYD8635M_LSCchecksum+=TYD8635M_LSCWriteData[i+62];
		}
		TYD8635M_LSCchecksum=TYD8635M_LSCchecksum%255+1;
	}

	TYD8635M_WriteLSCFlagSum();

	return TRUE;
}

void COtpTYD8635M::TYD8635M_WriteLSCFlagSum()
{
	Data_Write.data_Lenc[0] = FLAG_VALID;
	Data_Write.data_Lenc[1] = TYD8635M_LSCchecksum;
...
BOOL COtpTYD8635M::ProgrammingAF()
{
	if (m_LSCItem != 2)
	{
...
void COtpTYD8635M::Programming_3L8AF()
{
	int i = 0;
	BYTE WriteData[64] = {0};
	if (m_LscGroup == 1)
...
BOOL COtpTYD8635M::ProgrammingPDAF()
{
	BOOL bRet = TRUE;
	//MTK
	if (m_EnPDAF)				
...
BOOL COtpTYD8635M::TYD8635M_OTPVerify(void)
{
	if (!m_dummyburn)//!m_dummyburn
	{
		if (!TYD8635M_CheckOTPReadWrite())		return FALSE;
	}
	else 
	{
		if (m_LSCItem == 2)
...
BOOL COtpTYD8635M::TYD8635M_CheckOTPReadWrite(void)
{
...
BOOL COtpTYD8635M::Sensor3L8_CheckReadWrite()
{
	BYTE ReadData[64] = {0};
...
BOOL COtpTYD8635M::Sensor3L8_CheckLSCReadWrite()
{
	int i = 0;
	if (m_LscGroup == 1)
	{
		S5K3L8_ReadOTPPage(0x19,TYD8635M_LSCReadData);
		msleep(100);
		S5K3L8_ReadOTPPage(0x1A,TYD8635M_LSCReadData+64);
		msleep(100);
		S5K3L8_ReadOTPPage(0x1B,TYD8635M_LSCReadData+128);
		msleep(100);
		S5K3L8_ReadOTPPage(0x1C,TYD8635M_LSCReadData+192);
		msleep(100);
		S5K3L8_ReadOTPPage(0x1D,TYD8635M_LSCReadData+256);
		msleep(100);
		S5K3L8_ReadOTPPage(0x1E,TYD8635M_LSCReadData+320);
		msleep(100);
		S5K3L8_ReadOTPPage(0x1F,TYD8635M_LSCReadData+384);
		msleep(100);	
		for (i=31;i<(417+31);i++)
		{	
			if (TYD8635M_LSCReadData[i] != TYD8635M_LSCWriteData[i])
			{
	...
	if (m_LscGroup == 2)
	{
		S5K3L8_ReadOTPPage(0x12,TYD8635M_LSCReadData);
		msleep(100);
		S5K3L8_ReadOTPPage(0x13,TYD8635M_LSCReadData+64);
		msleep(100);
		S5K3L8_ReadOTPPage(0x14,TYD8635M_LSCReadData+128);
		msleep(100);
		S5K3L8_ReadOTPPage(0x15,TYD8635M_LSCReadData+192);
		msleep(100);
		S5K3L8_ReadOTPPage(0x16,TYD8635M_LSCReadData+256);
		msleep(100);
		S5K3L8_ReadOTPPage(0x17,TYD8635M_LSCReadData+320);
		msleep(100);
		S5K3L8_ReadOTPPage(0x18,TYD8635M_LSCReadData+384);
		msleep(100);
		S5K3L8_ReadOTPPage(0x19,TYD8635M_LSCReadData+448);
		msleep(100);
		for (i=62;i<(417+62);i++)
		{	
			if (TYD8635M_LSCReadData[i] != TYD8635M_LSCWriteData[i])
...
BOOL COtpTYD8635M::TYD8635M_VerifyLSC()
{
...
BOOL COtpTYD8635M::TYD8635M_VerifyAWB()
{
	if (!GetAWBInfo(&m_wbCurrent))		return FALSE;
	if (!TYD8635M_CheckAWB(m_wbCurrent,2))		return FALSE;
	return TRUE;
}