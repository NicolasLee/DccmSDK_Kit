...
// NL 对标度信 imagekit.h，稍作修改作为自定义imagekit.
...
typedef struct _SensorTab
{
	...
	TCHAR name[64];       /// < name of the SENSOR
	...
	int avdd;              ///<SENSOR的AVDD电压值
	/// @brief SENSOR的DOVDD电压值
	int dovdd;             ///<SENSOR的DOVDD电压值
	/// @brief SENSOR的DVDD电压值		
	int dvdd;							///<SENSOR的DVDD电压值
	int afvdd;
	int vpp;
	/// @brief SENSOR的数据接口类型
...
}SensorTab;//紧随SensorTab, 增加SensorTab2定义
